<template>
    <div v-if="getOrders" class="container main-layout">
        <h1 style="margin:20px 0">My Trips</h1>
        <table class="responsive-table">
            <thead>
                <tr>
                    ...
                    <th scope="col">Vaction date</th>
                    <th scope="col">Status</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="order in getOrders">
                    <td data-title="Date">{{ foramtCreatedAt(order.createdAt) }}</td>
                    <td data-title="Host name">{{ order?.host.fullname }}</td>
