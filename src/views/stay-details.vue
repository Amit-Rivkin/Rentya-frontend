    ...
    <section v-if="stay" class="details-layout details-page">
        ...
        <div class="mobile-reserve-bottom">
            <div class="flex space-between align-items-center">
                ...
                <p class="date">{{fomratStayDate}}</p>
                </div>
                <button @click="onAddOrder" class="bnb-btn">Reserve</button>

            </div>
        </div>
        <section class="modal-area" v-if="isReserveModal">
            <div class="grey-underline">
                <h1 class="modal-title">Your order has been sent</h1>
            </div>
            <div class="content-area grey-underline">
                <h1 class="reservtion-details">Reservation details</h1>
                <div class="details-grid">

                <h1 class="details-title">Your host: <span class="details-info"> {{stay.host.fullname}}</span></h1>
                <h1 class="details-title">Total Nights: <span class="details-info"> {{nightsAmount}}</span></h1>
                <h1 class="details-title">Total Price: <span class="details-info"> ${{this.stay.price * this.stayLength + (this.stayLength * 25)}}</span></h1>
                <h1 class="details-title">Dates: <span class="details-info"> {{fomratStayDate}} </span></h1>
                </div>
                 <image-gallery :images="stay.imgUrls"></image-gallery>
                <h1 class="content-txt">Thank you for choosing rentya</h1>
            </div>
...
export default {
    ...
    computed: {
        ...
        getRatingAvg() {
         ...
         nightsAmount() {
            const startDate = new Date(this.stayDate[0])
            const endDate = new Date(this.stayDate[1])
            const diffTime = Math.abs(endDate - startDate);
            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
